...
{
 return ((CUR Strings[105] != 0) ? sp->_dch_cost : ((CUR Strings[21] != 0) ? (sp->_dch1_cost * count) : 1000000));
}

 int
_nc_InsCharCost_sp (SCREEN *sp, int count)
{
 return ((CUR Strings[108] != 0) ? sp->_ich_cost : ((CUR Strings[31] && CUR Strings[42]) ? sp->_smir_cost + sp->_rmir_cost + (sp->_ip_cost * count) : ((CUR Strings[52] != 0) ? ((sp->_ich1_cost + sp->_ip_cost) * count) : 1000000)));
}
